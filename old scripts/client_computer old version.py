...
def gendata(iden, i):
    try:
        return np.genfromtxt(iden, delimiter = ',', skip_header = i, skip_footer = 1)
    except:
...
def main():
    ...
    new_iter = Volume_data1211.size
    i = Volume_data1211.size
    new_points = new_iter - i
    i = new_iter
    try:
        send_commands(conn, cmd = f'QD_data {Volume_data1211[-1]} {Rate_of_Change1211[-1]} {P_dewar1211[-1]} {SP_press1211[-1]} {Volume_data1322[-1]} {Rate_of_Change1322[-1]} {P_dewar1322[-1]} {SP_press1322[-1]} {connection1211[-1]} {connection1322[-1]}')
    except:
        conn = checkconnection(conn)
        send_commands(conn, cmd = f'QD_data {Volume_data1211[-1]} {Rate_of_Change1211[-1]} {P_dewar1211[-1]} {SP_press1211[-1]} {Volume_data1322[-1]} {Rate_of_Change1322[-1]} {P_dewar1322[-1]} {SP_press1322[-1]} {connection1322[-1]} {connection1322[-1]}')
    ...
    while True:
        ...
        if timedelta.total_seconds() >= 60:
            ...
            new_points = new_iter - i
            i = new_iter
            print(f'QD_data {Volume_data1211[-1]} {Rate_of_Change1211[-1]} {P_dewar1211[-1]} {SP_press1211[-1]} {Volume_data1322[-1]} {Rate_of_Change1322[-1]} {P_dewar1322[-1]} {SP_press1322[-1]} {connection1211[-1]} {connection1322[-1]}')
            try:
                send_commands(conn, cmd = f'QD_data {Volume_data1211[-1]} {Rate_of_Change1211[-1]} {P_dewar1211[-1]} {SP_press1211[-1]} {Volume_data1322[-1]} {Rate_of_Change1322[-1]} {P_dewar1322[-1]} {SP_press1322[-1]} {connection1211[-1]} {connection1322[-1]}')
            except:
                conn = checkconnection(conn)
